...
export default {
  ...
  watch: {
    async visible(flag) {
      if (flag) {
        await waitUnitl(() => this.$refs.ctx);
        this.dom = this.$refs.ctx;
        this.dom && console.log("dom1: ", this.dom.getBoundingClientRect());

        if (!this.dom) {
            await waitUnitl(() => this.$refs.ctx);
            this.dom = this.$refs.ctx;
            this.dom && console.log("dom2: ", this.dom.getBoundingClientRect());
        }
      }
    },
  },
